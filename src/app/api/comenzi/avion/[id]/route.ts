...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const row = await prisma.comandaAvion.update({
      where: { id },
      data: { furnizor, produse, cantitate, awb, dataPlecare, dataSosireEstimata, valoare, status },
    });
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  try {
    await prisma.comandaAvion.delete({ where: { id } });
